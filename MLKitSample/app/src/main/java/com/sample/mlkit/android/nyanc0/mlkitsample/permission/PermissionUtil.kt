...
class PermissionUtil constructor(private val activity: Activity) {
     ...
     */
    fun isAuthrized(permission: Permission): Boolean {
     ...
     */
    fun isAuthrized(permissions: Array<Permission>): Boolean {
        var granted = true
        permissions.forEach { granted = granted.and(isAuthrized(it)) }
        return granted
    }
